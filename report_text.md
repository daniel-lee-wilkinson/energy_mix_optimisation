...
*   **Data Files:** Generates CSV files containing:
    *   Processed weather data and *potential* generation per MW.
    *   Hourly supply profile (PV, Wind, Grid, Demand) for the *optimal* mix.
*   **Visualizations:** Creates PNG plots illustrating:
    *   Raw generation potential and temperature profiles.
    *   Monthly energy mix breakdown (PV/Wind/Grid vs Demand).
    *   Optimal energy mix pie chart.
    *   Temperature distribution and impact analysis.
    *   GWP variation by temperature range.
    *   Hourly supply/demand profiles for representative weeks.

## 4. Key Results (Illustrative - based on a typical run)

*(Note: Specific numerical results depend on the exact input parameters and weather data used. The following describes the types of results generated)*

*   **Optimal Capacities:** The optimization identifies specific capacities (e.g., X MW of PV and Y MW of Wind).
*   **Land Use:** The corresponding land required for PV (e.g., X * 0.02 km²) and Wind (e.g., Y * 0.4 km²) is calculated, along with the total, confirming it is within the available limit.
*   **Energy Mix:** The analysis provides the annual percentage contribution of PV, Wind, and Grid to meet the total demand (e.g., 45% PV, 35% Wind, 20% Grid).
*   **GWP:** The minimized GWP value for the optimal mix is reported (e.g., 0.15 kg CO2e/kWh).
*   **Stability:** Metrics indicate the reliance on the grid (e.g., "Grid required for 30% of hours," "Max hourly deficit of Z kWh").
*   **Visual Insights:** Plots provide visual understanding of seasonal generation patterns, monthly mix variations, and the relationship between temperature, demand, and GWP.
