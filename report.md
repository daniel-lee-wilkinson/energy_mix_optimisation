...
*   **Primary Weather Data:** Hourly weather data (Solar Irradiation `ALLSKY_SFC_SW_DWN`, Wind Speed `WS2M`, Temperature `T2M`) sourced from NASA POWER API for the central Australian site for the year 2023. Site coordinates are omitted for privacy but were used for the API request.
*   **Fallback:** If primary site data is unavailable, alternate remote Australian location data or synthetic data is used.
...
*   **Data Files:** Generates `output_data/australian_energy_data_potentials.csv` (hourly potential per MW for the analysed site) and `output_data/optimal_supply_profile.csv` (hourly results for the optimal mix, including battery state).
...
## 4. Results (Example: 10,000 MWh Demand / 1000 km² Land at Central Australian Site)

*(Results below are for the specific scenario analysed: powering a CO2 removal system requiring 10,000 MWh/year within 1000 km² at the central Australian site, using the hourly simulation model)*
...
For the analysed scenario (10,000 MWh demand, 1000 km² land at the central Australian site, minimizing GWP/kWh), the model consistently favors maximizing PV installation up to the land limit. This occurs despite wind's significantly lower GWP factor (0.011 vs 0.07 kg CO2e/kWh) primarily because:
1.  **Land Efficiency:** PV requires far less land per MW (0.02 km²/MW) compared to wind (0.26 km²/MW), approximately 13 times less.
2.  **Generation Profile & Yield:** At this central Australian site, the modelled annual energy yield per km² of land dedicated appears substantially higher for PV than for wind under the utilised generation models. A direct comparison shows PV yielded ~5.2 MWh/km²/year (5219 MWh / 1000 km²) in the final configuration, whereas dedicating the same land to wind would yield significantly less total energy based on its lower land efficiency and site wind profile.
